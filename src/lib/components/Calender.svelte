...
<div class="p-6 border-2 border-opacity-20 rounded-2xl h-min shadow-lg shadow-gray-700/40">
    <div class="flex justify-between p-1 mb-6">
        <div class="font-semibold">{monthName} <span class="pl-1 text-gray-500 text-opacity-50 font-bold">{currentYear}</span></div>
    ...
    <div class="grid grid-cols-7 h-min text-lg font-normal">
        ...
        {#each currentMonthDays as _, i}
            {#if i == dt.day && isNow}
                <div class="w-28 h-28 border-2 rounded border-opacity-50 border-gray-100 transition-all hover:scale-95 hover:bg-gray-900 hover:bg-opacity-20 shadow-lg shadow-gray-600/40">
